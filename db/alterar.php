...
if($_GET['codigo']) {
    ...
    if ($stmt->execute()){ 
        ...
        if($resultado->num_rows > 0){  
            ...
            if($dados['nascimento']){
                $dt = new DateTime($dados['nascimento']);               //precisa tratar porque o SQL lê data diferente
                $dados['nascimento'] = $dt->format('d/m/Y');
            }
            if($dados['salario']){
                $dados['salario'] = str_replace(".", ",", $dados['salario']);    //substitui o "." pelas "," na saida
            }
...
if (count($_POST) > 0) {
    ...
    if(!count($erros)) {  //se o count for 0, ou seja, se NÃO tiver ERROS, então...
        ...
        $params = [
            ...
            $dados['site'],
            $dados['filhos'],
            $dados['salario'] ? str_replace(",", ".", $dados['salario']) : null, //se tiver vírgula, colocar ponto para o sql ler
            $dados['id']
        ];

        $stmt->bind_param("ssssidi", ...$params); #4 strings, 1 inteiro, 1 decimal e 1 inteiro e os parametros passados
...
<!-- dir=db&file=alterar&codigo=10 // esconder o dir=db, file=alterar-->
