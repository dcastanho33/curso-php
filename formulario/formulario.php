...
if (count($_POST) > 0) {
    ...
    //if isset($_POST['nome']);
    if (!filter_input(INPUT_POST, "nome")) {      //se NÃO tiver input no post então
        $erros['nome'] = "Nome é obrigatório";
    }

    if (filter_input(INPUT_POST, "nascimento")) {      //se tiver input nascimento no post então
    ...
    if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) { //se a variável do filtro não for válida
